...
const MobileNavItem = ({ item, pathname }: { item: NavItem; pathname: string }) => {
  ...
  return (
    <div className="w-full">
      ...
      >
        <span className={isActive && !hasChildren ? 'font-semibold' : ''}>{item.title}</span>
        {hasChildren && (
          <svg
          ...
          {item.children?.map((child) => (
            <Link
              ...
              className={`block px-4 py-2 text-sm text-on-primary/90 hover:bg-accent/20 hover:text-accent rounded-md transition-colors duration-200 ${
                pathname === child.href ? 'font-medium bg-accent/10 text-accent' : ''
              }`}
            >
              {child.title}
            </Link>
                  ...
                  <div key={item.title} className="relative group">
                    <button
                      className={`px-4 py-2 rounded-md flex items-center space-x-1 ${
                        isActive ? 'bg-primary-hover' : 'hover:bg-primary-hover/50'
                      }`}
                    >
              ...
              <div className="relative w-6 h-5">
                ...
                <span 
                  className={`absolute left-0 w-6 h-0.5 bg-current transition-opacity duration-200 ${
                    isOpen ? 'opacity-0' : 'opacity-100 top-1/2 -translate-y-1/2'
                  }`}
                  aria-hidden="true"
                />
                <span 
