...
export default function Connect4Board({ gameState, makeMove }: GameBoardProps) {
  ...
  const isYourTurn =
    gameState.status == 'in_progress' && gameState.turn === yourIdx;
  ...
  return (
    <div className="w-full bg-primary p-2 rounded-lg grid grid-cols-7 gap-2">
      {[...Array(7)].map((_, col) => (
        <div
          key={`col-wrapper-${col}`}
          className="relative"
        ...
        >
          {hoveredCell === col && (
            <div
              ...
              ${
                isYourTurn ? 'secondary' : 'background'
              }) 0%, transparent 100%)`,
              }}
            />
          )}
